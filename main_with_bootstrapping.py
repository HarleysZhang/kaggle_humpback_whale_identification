...
BOOTSTRAP = os.path.join(src_dir, "data/bootstrap.pickle")
...
tagged_new = dict(tagged, **bootstrapping)
tagged = tagged_new
h2ws = {}
new_whale = 'new_whale'
...
# 											  verbose=2)
# 	score = head_model.predict_generator(ScoreGen(features, verbose=verbose), max_queue_size=12, workers=6, verbose=2)
...
def make_steps(step, ampl):
	...
	with open('model_log.txt', "a") as f:
		f.write(log_str)  # 将字符串以'a'模式写入文本文件
...
# def write_log(filename, str_log):
# 	'''
# 	write train log in filename
# 	'''
...
if isfile('./models/standard_epoch350.h5'):
	...
	with open('model_log.txt', "a") as f:
		start_str = '\n[Siamese========lapjv========Size512*3========Epoch350========MultiThreads]\n'
		f.write(start_str)
		f.write('\nEpoch===================================================================>10\n')
	...
	model.save_weights('./models/standard_epoch110.model', overwrite=True)

	model.load_weights('./models/standard_epoch110.h5')
	print('Load standard_epoch110.h5 success!')
	steps = 110
	# epoch -> 135
	with open('model_log.txt', "a") as f:
		f.write('\nEpoch===================================================================>135')
	...
	model.save_weights('./models/standard_epoch135.h5', overwrite=True)
	...
	model.save_weights('./models/standard_epoch155.h5', overwrite=True)
	...
	# # epoch -> 180
	# for _ in range(3): make_steps(5, 0.25)
	# model.save_weights('./models/standard_epoch180.h5', overwrite=True)
	...
	model.save_weights('./models/standard_epoch280.h5', overwrite=True)
	...
	model.save_weights('./models/standard_epoch320.h5', overwrite=True)
	...
	model.save_weights('./models/standard_epoch330.h5', overwrite=True)
	...
	# epoch -> 340
	for _ in range(2): make_steps(5, 0.25)
	model.save_weights('./models/standard_epoch340.h5', overwrite=True)
