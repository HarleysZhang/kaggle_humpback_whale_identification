# encoding：utf-8
# filename: test_tta.py
# function: 模型测试程序,测试数据增强
...
class FeatureGen(Sequence):
	...
	def __getitem__(self, index):
		...
		a = np.zeros((size,) + img_shape, dtype=K.floatx())
		for i in range(size): a[i, :, :, :] = read_for_training(self.data[start + i])
		if self.verbose > 0:
...
if isfile('./models/standard_epoch350.h5'):
	# tmp = keras.models.load_model('./models/standard_epoch400.model')
...
for i in range(4):
	...
	prepare_submission(0.99, './submission_Local_Boot_Epochs350_512size_tta%d.csv' % i)
	toc = time.time()
	print("%d tta submission files take : %.3f minute" % (i, (toc - tic) / 60.))
